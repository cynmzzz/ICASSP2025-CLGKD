...
parser.add_argument('--data', default='/data/dataset/', type=str, help='Dataset directory')
...
def train(epoch, criterion_list, optimizer):
    ...
    class_acc1 = [round((class_top1_num[i]/(total)).item(), 4) for i in range(num_auxiliary_branches)] + [round((top1_num/(total)).item(), 4)]
    class_acc5 = [round((class_top5_num[i]/(total)).item(), 4) for i in range(num_auxiliary_branches)] + [round((top5_num/(total)).item(), 4)]
...
def test(epoch, criterion_cls):
    ...
    with torch.no_grad():
        ...
        print('test epoch:{}\tTest Top-1 class_accuracy: {}\n'.format(epoch, str(class_acc1)))
