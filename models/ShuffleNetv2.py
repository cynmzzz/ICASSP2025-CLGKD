...
class Auxiliary_Classifier(nn.Module):
    def __init__(self, net_size, num_classes=100):
        ...
        self.block_extractor1 = nn.Sequential(*[self._make_layer(out_channels[1], num_blocks[1]),
        ...
        self.block_extractor2 = nn.Sequential(*[self._make_layer(out_channels[2], num_blocks[2]),
                                                nn.Conv2d(out_channels[2], out_channels[3],
                                                kernel_size=1, stride=1, padding=0, bias=False),
                                                nn.BatchNorm2d(out_channels[3]),
                                                nn.ReLU(inplace=True)])
        ...
        self.block_extractor3 = nn.Sequential(*[self._make_layer(out_channels[2], num_blocks[2], stride=1),
                                                nn.Conv2d(out_channels[2], out_channels[3],
                                                kernel_size=1, stride=1, padding=0, bias=False),
                                                nn.BatchNorm2d(out_channels[3]),
                                                nn.ReLU(inplace=True)])
...
class ShuffleNetV2_Auxiliary(nn.Module):
    def __init__(self, net_size, num_classes=100):
        ...
        self.auxiliary_classifier = Auxiliary_Classifier(net_size, num_classes=num_classes)
...
if __name__ == '__main__':
    ...
    net = ShuffleV2_aux(num_classes=100)
